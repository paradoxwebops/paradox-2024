...
const EventSubHeading = ({ children }: { children: React.ReactNode }) => {
  return (
    <h2
      className={`${inter.className} text-white font-semibold text-3xl mb-4 mt-4`}
      style={{
        display: "list-item",
        listStyleImage: 'url("/eventHeadingPrefix.png")',
...
const EventPill = ({
  ...
  return (
    <Card
      bgColor={rgba("#d9d9d9", 0.25)}
      className={`text-[#A36FC4] py-3 flex items-center ${className}`}
      style={{ width: small ? "auto !important" : "100%" }}
    >
      {children}
    </Card>
  );
...
const EventJudgeCard = ({ name, email, dude_type, bio }: EventPerson) => {
  return (
    <Card bgColor={rgba("#ffffff", 0.1)} className="text-white">
      <Card
        bgColor="#301042"
        className="!w-auto !py-1 !px-3 text-sm font-semibold"
        style={{ display: "inline-block" }}
      >
        {correctText(dude_type)}
      </Card>
      ...
      {bio && (
        <div>
          <small className="whitespace-pre-line">{bio}</small>
        </div>
      )}
    </Card>
  );
    ...
    <Card bgColor={rgba("#230137", 1)} className="">
      <div className="flex items-center gap-4">
        ...
        <div className="w-full">
          ...
          <Card
            bgColor="#fff"
            className="!w-[fit-content] !py-1 !px-2 text-sm font-semibold my-3"
            style={{ display: "inline-block" }}
          >
            {correctText(title)}
          </Card>
                    ...
                    {pills.map(({ t, v }, index) => {
                      return (
                        <EventPill
                          ...
                          key={`${start_time}_${end_time}_${ind}_${index}`}
                        >
                          <div className="lg:flex items-center justify-between w-full">
                            ...
                            {t == "Location" ? (
                              <Link
                                href={location_link}
                                target="_blank"
                                className="lg:w-1/2 text-right max-lg:text-sm"
                              >
                                {v}
                              </Link>
                            ) : (
  ...
  return (
    <Card bgColor="#230137" className="p-12 my-6">
      <Card
        bgColor=""
        style={{
          backgroundColor: rgba("#d9d9d9", 0.1),
      ...
      >
        ...
        <div className="flex max-md:flex-col gap-4">
          ...
          {is_team_event ? (
            <>
              <EventPill small={true}>
                <b>Max</b>: {max_team_size}
              </EventPill>
              <EventPill small={true}>
                <b>Min</b>: {min_team_size}
              </EventPill>
            </>
          ) : (
            <>
              {max_participants !== "" ||
                (max_participants !== null && (
                  <EventPill small={true}>
                    <b>Maximum participants</b>: {max_participants}
                  </EventPill>
                ))}
            </>
          )}
        ...
        <EventPill className="!text-white max-md:flex-col justify-between mt-4">
          <Link
            href={location_link || "#"}
            target="_blank"
            className="flex justiy-center items-center gap-2"
          >
            <span>
              <b>Location</b>:
            </span>
            <span>{location ? location.toString() : "N/A"}</span>
          </Link>
        </EventPill>
          ...
          <>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 md:px-12 gap-4">
              {sponsors?.map((sponsor, ind) => {
                return <EventSponsorCard {...sponsor} key={ind} />;
              })}
            </div>
          </>
        )}
        ...
        {some_dudes.length >= 1 && (
          <>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 md:px-12 gap-4">
              {some_dudes?.map((dude, ind) => {
                return <EventJudgeCard {...dude} key={ind} />;
              })}
            </div>
          </>
        )}
        ...
        <>
          <EventSubHeading>Contact</EventSubHeading>
          <p className="text-white md:px-12">{head}</p>
        </>
      </Card>
    </Card>
  );
